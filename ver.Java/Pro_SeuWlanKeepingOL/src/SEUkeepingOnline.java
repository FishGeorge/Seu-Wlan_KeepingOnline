...
public class SEUkeepingOnline {
    ...
    private String SeuLogoutURL = "http://w.seu.edu.cn/index.php/index/logout";
     ...
     */
    public int Logout() {
        System.out.println(df.format(new Date()) + " Try to logout...");
        // 检测外网连接情况
        if (!URL_ConnectiveCheck(BingURL)) {
            System.out.println(df.format(new Date()) + " Has been logout");
            return -1;
        } else {
            try {
                URL url = new URL(SeuLogoutURL);
                conn = (HttpURLConnection) url.openConnection();
                // 命令类型 POST
                conn.setRequestMethod("POST");
                // 添加POST Header
    ...
    /*
     * 用于测试某个URL连接正常
     * param: int seconds 连接成功后的检查网络周期
     * return: null
     *
     */
